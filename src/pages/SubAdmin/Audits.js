		...
		Endpoint.getAudits()
			.then((res) => {
				...
				let mappedData = res.data.map((faculty, i) => {
					return{
						sNo: i + 1,
						actionPerformed:  <span className="capital" >{faculty.actionPerformed}</span>,
                        performedby: <span className="capital" >{faculty?.user?.person?.email}</span>,
                        actionTime: <span className="capital" >{faculty.actionTime}</span>,
						...
						// 			<Unicons.UilUsersAlt size="19"/> System Audits
						// 		</Link>
						// 	</div>
					}
				});
