...
class SchoolAdminSession extends Component {
	...
	confirmEditSemester = (data) => {
		this.setState({semesterEdit: !this.state.semesterEdit, name: data.name, semesterId: data.id})
	}

	confirmEditSession = (data) => {
		this.setState({sessionEdit: !this.state.sessionEdit, name: data.name, sessionId: data.id})
	}
	...
	dynamicSuccess = (data) => toast.success(data, {
		style: {
			border: '1px solid #56b39d',
			padding: '16px',
			background: '#56b39d',
	...
	executeEditSemester = () => {
		this.setState({newSessionLoading:true})
		Endpoint.modifySemester(this.state.semesterId, this.state.name)
		.then((res) => {
			console.log(res)
		this.setState({newSessionLoading:false})
		this.loadDataFromServer()
			this.dynamicSuccess("Modified successfully!")
		})
		.catch((err) => {
			console.log(err)
			this.setState({newSessionLoading:false})
	...
	executeEditSession = () => {
		this.setState({newSessionLoading:true})
		Endpoint.modifySession(this.state.sessionId, this.state.name)
		.then((res) => {
			console.log(res)
		this.setState({newSessionLoading:false})
		this.loadDataFromServer()
	...
	render() {
		return (
			<>
													...
													<tbody>
															...
															this.state.allSessions && this.state.allSessions.map((session, index) => {
																return (
																	...
																	<tr>
																		<td>{session.name}</td>
																		<td><button onClick={() => this.confirmEditSession(session)} type='button' className='btn btn-outline-success btn-sm'><i className='fa fa-edit'/></button></td>
																	</tr>
																	</>
																);
														})}
											...
											<div className="col-md-6">
												...
												<table className='table'>
													...
													<tbody>
															...
															{
															this.state.allSemesters && this.state.allSemesters.map((semester, index) => {
																return (
																	<>
																	<tr>
																		<td>{semester.name}</td>
																		<td><button onClick={() => this.confirmEditSemester(semester)} type='button' className='btn btn-outline-success btn-sm'><i className='fa fa-edit'/></button></td>
																	</tr>
																	</>
																);
														})}
				...
				<Modal isOpen={this.state.semesterEdit} toggle={this.confirmEditSemester} className="mt-5 md">
					<form>
						<ModalHeader toggle={this.confirmEditSemester}>
							<span className="h2">Edit Semester</span>
						</ModalHeader>
						...
						<ModalFooter>
							<button className="btn btn-primary" 
							onClick={this.executeEditSemester}
							type="button"
							...
							<button type="button" className="btn btn-danger" onClick={this.confirmEditSemester}>Close</button>
						</ModalFooter>
					</form>
				</Modal>

				<Modal isOpen={this.state.sessionEdit} toggle={this.confirmEditSession} className="mt-5 md">
					<form >
						<ModalHeader toggle={this.confirmEditSession}>
							<span className="h2">Edit Session</span>
						</ModalHeader>
						...
						<ModalFooter>
							<button className="btn btn-primary" 
							type='button'
							onClick={this.executeEditSession}
							>
								Save
								{
									this.state.newSessionLoading ?
							...
							<button type="button" className="btn btn-danger" onClick={this.confirmEditSession}>Close</button>
						</ModalFooter>
					</form>
				</Modal>
			</>
