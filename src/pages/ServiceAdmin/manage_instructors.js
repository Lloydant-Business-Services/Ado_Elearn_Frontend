...
class ManageInstructors extends Component {
  state = {
    pageLoading: false,
  ...
  addSubInstructorSuccess = () =>
    toast.success("Sub Instructor added successfully", {
      style: {
        border: "1px solid #56b39d",
        padding: "16px",
        background: "#56b39d",
  ...
  removeSubInstructorSuccess = () =>
    toast.success("Sub Instructor removed successfully", {
      style: {
        border: "1px solid #56b39d",
        padding: "16px",
        background: "#56b39d",
  ...
  assignCourseExists = () =>
    toast.error("Course already assigned for current semester", {
      style: {
  ...
  addSubInstructorExists = () =>
    toast.error("The Sub Instructor has already been added to this course", {
      style: {
        border: "1px solid #FFCC00",
        padding: "16px",
        background: "#FFCC00",
  ...
  openNewSubInstructor = (course) => {
    this.setState({
      newSubInstructor: true,
      newSubInstructorCourse: course,
      newSubInstructorCourseId: course.courseAllocationId,
    });
  };

  openGetSubInstructors = (course) => {
    this.setState({
      getSubInstructor: true,
      currentCourse: course,
      subInstructorsLoading: true,
    });
    Endpoint.getCourseSubInstructors(course.courseId)
      .then((res) => {
        console.log(res.data);
        this.setState({
          currentCourseSubInstructors: res.data,
          subInstructorsLoading: false,
        });
      })
      .catch((error) => {
  ...
  assignCourse = (e) => {
    e.preventDefault();
    if (
      !this.state.assignCourseId ||
      !this.state.assignInstructorId ||
      !this.state.assignLevelId
    ) {
  ...
  addNewSubInstructor = (e) => {
    e.preventDefault();
    if (
      !this.state.addSubInstructorId ||
      !this.state.newSubInstructorCourseId
    ) {
      this.setState({
        error: true,
        errorMessage: "Please Select a Sub Instructor to add.",
      });
    ...
    this.setState({
      addNewSubInstructorLoading: true,
      success: false,
      error: false,
    });
    ...
    Endpoint.addSubInstructor(addSubInstructorProps)
      .then((res) => {
        ...
        if (res.data.statusCode === 200) {
          ...
          this.setState({
            addSubInstructorId: null,
            newSubInstructorCourseId: null,
            addNewSubInstructorLoading: false,
            newSubInstructor: false,
          });
      ...
      .then((res) => {
        ...
        Endpoint.getInstructorsByDepartment(res.data.department.id)
          .then((res) => {
            ...
            this.setState({
              allInstructors: res.data,
              allInstructorNames: newInstructorObj,
              pageLoading: false,
            });
          })
          .catch((error) => {
          ...
          .then((res) => {
            ...
            let mappedData = res.data.map((course, i) => {
              return {
                sNo: i + 1,
                title: (
                  <span searchvalue={course.courseTitle} className="capital">
                    {course.courseTitle}
                  </span>
                ),
                ...
                actions: (
                  <div>
                    <button
                      ...
                      onClick={() => this.openNewSubInstructor(course)}
                    >
                    ...
                    <button
                      ...
                      onClick={() => this.openGetSubInstructors(course)}
                    >
  ...
  render() {
    return (
      <>
          ...
          <div className="d-flex flex-wrap justify-content-between">
            <div>
              <div className="d-flex">
                ...
                >
                  <span>
                    After creating a course, assign it to a departmental
                    instructor to see it in the list.
                  </span>
                </ReactTooltip>
              </div>
            </div>
        ...
        <Modal
          isOpen={this.state.newCourse}
          toggle={this.toggleNewCourse}
          className="mt-5 md"
          size="lg"
        >
                    ...
                    {this.state.newCourseFormIncomplete ? (
                      <div className="bg-danger border-rad-full text-center p-2 mb-3 custom-form-alert">
                        <p className="small text-white mb-0">
                          <Unicons.UilExclamationCircle size="20" /> Please fill
                          all fields.
                        </p>
                      </div>
                    ) : null}
        ...
        <Modal
          isOpen={this.state.assignCourse}
          toggle={this.toggleAssignCourse}
          className="mt-5 md"
          size="lg"
        >
          <form onSubmit={(e) => this.assignCourse(e)}>
              ...
              <div className="form-group row">
                ...
                <div className="col-md-6">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      this.setState({ assignLevelId: e.target.value })
                    }
                  >
            ...
            <ModalFooter>
              ...
              <button
                type="button"
                className="btn btn-danger"
                onClick={this.toggleAssignCourse}
              >
                Close
              </button>
            </ModalFooter>
        ...
        <Modal
          isOpen={this.state.newSubInstructor}
          toggle={this.toggleNewSubInstructor}
          className="mt-5 md"
          size="sm"
        >
          <form onSubmit={(e) => this.addNewSubInstructor(e)}>
            <ModalHeader toggle={this.toggleNewSubInstructor}>
              <span className="h2">
                Add Sub Lecturer to{" "}
                {this.state.newSubInstructorCourse.courseTitle} [
                {this.state.newSubInstructorCourse.courseCode}]
              </span>
            </ModalHeader>
            ...
            <ModalFooter>
              ...
              <button
                type="button"
                className="btn btn-danger"
                onClick={this.toggleNewSubInstructor}
              >
                Close
              </button>
            </ModalFooter>
        ...
        <Modal
          isOpen={this.state.getSubInstructor}
          toggle={this.toggleGetSubInstructor}
          className="mt-5 md"
          size="lg"
        >
          <ModalHeader toggle={this.toggleGetSubInstructor}>
            <span className="h2">
              {this.state.currentCourse.courseTitle} |{" "}
              <span className="h4">Sub Instructors</span>
            </span>
          </ModalHeader>
            ...
            <div className="table-responsive">
              <table className="table table-hover table-striped">
                ...
                <tbody>
                  ...
                  ) : this.state.currentCourseSubInstructors &&
                    this.state.currentCourseSubInstructors.length ? (
                    this.state.currentCourseSubInstructors.map(
                      (instructor, index) => {
                        return (
                          <tr key={instructor.id}>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() =>
                                  this.removeSubInstructor(instructor)
                                }
                              >
                    ...
                    <tr>
                      <td colSpan="3">
                        <p className="text-center">
                          No SubInstructors for this course.
                        </p>
                      </td>
                    </tr>
                  )}
          ...
          <ModalFooter>
            <button
              type="button"
              className="btn btn-danger"
              onClick={this.toggleGetSubInstructor}
            >
              Close
            </button>
          </ModalFooter>
